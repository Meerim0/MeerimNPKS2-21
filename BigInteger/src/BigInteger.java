...
public class BigInteger {
    ...
    public BigInteger(List<Integer> list) {
        this.list = new ArrayList<>(list); // инициализируем list копией переданного списка
    }
    ...
    public static ArrayList<Integer> addBigInt(BigInteger a, BigInteger b) {
        ...
        while (itA >= 0 || itB >= 0 || carry != 0) {
            ...
            carry = sum / 10; // рассчитываем новое значение переноса как целая часть от деления суммы на 10
    ...
    public static ArrayList<Integer> subtractBigInt(BigInteger a, BigInteger b) {
        ArrayList<Integer> aList = a.getList();
        ArrayList<Integer> bList = b.getList();
        ArrayList<Integer> result = new ArrayList<>();
        ...
        while (result.size() > 1 && result.get(result.size() - 1) == 0) { // удаляем ведущие нули
    ...
    public static ArrayList<Integer> multiplyBigInt(BigInteger a, BigInteger b) {
        ArrayList<Integer> aList = a.getList();
        ArrayList<Integer> bList = b.getList();
        int m = aList.size();
        int n = bList.size();
        ...
        for (int i = m - 1; i >= 0; i--) {
            ...
            for (int j = n - 1; j >= 0; j--) {
                ...
                result.set(i + j + 1, mul % 10); // обновляем значение в result
    ...
    public static ArrayList<Integer> divideBigInt(BigInteger a, BigInteger b) {
        ...
        for (int i = 0; i < dividend.size(); i++) {
            ...
            int quotientDigit = 0;
            while (compareBigInt(new BigInteger(remainder), b) >= 0) {
                remainder = subtractBigInt(new BigInteger(remainder), b);
                quotientDigit++;
            }
    ...
    private static int compareBigInt(BigInteger a, BigInteger b) {
        ArrayList<Integer> aList = a.getList();
        ArrayList<Integer> bList = b.getList();

        if (aList == null || bList == null) {
            throw new IllegalArgumentException("1 или оба операнда содержат 0ые списки");
        }

        int sizeA = aList.size(); // сравниваем размеры списков
        int sizeB = bList.size();

        if (sizeA != sizeB) { // если длины не равны
            return Integer.compare(sizeA, sizeB); // возвращаем результат сравнения длин
        }

        for (int i = 0; i < sizeA; i++) { // цикл для сравнения элементов
